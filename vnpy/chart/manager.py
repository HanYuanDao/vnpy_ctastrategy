...
class BaseDataManager(ABC):
    ...
    @abstractmethod
    def update_history(self, history: List[BaseData]) -> None:
        """
    ...
    @abstractmethod
    def update_bar(self, bar: BaseData) -> None:
        """
...
class TickManager:
    ...
    def update_history(self, history: List[TickData]) -> None:
        """
        Update a list of tick data.
        """
        # Put all new bars into dict
        for tick in history:
            self._ticks[tick.datetime] = tick

        # Sort bars dict according to bar.datetime
        self._ticks = dict(sorted(self._ticks.items(), key=lambda tp: tp[0]))
    ...
    def update_bar(self, tick: TickData) -> None:
        """
        Update one single tick data.
        """
        dt = tick.datetime
    ...
    def get_price_range(self, min_ix: float = None, max_ix: float = None) -> Tuple[float, float]:
        ...
        tick_list = list(self._ticks.values())[min_ix:max_ix + 1]
        first_bar = tick_list[0]
        max_price = first_bar.ask_price_5
        min_price = first_bar.bid_price_5

        for tick in tick_list[1:]:
            max_price = max(max_price, tick.ask_price_5)
            min_price = min(min_price, tick.bid_price_5)
    ...
    def get_volume_range(self, min_ix: float = None, max_ix: float = None) -> Tuple[float, float]:
        ...
        max_volume = first_tick.volume
        min_volume = 0

        for tick in tick_list[1:]:
            max_volume = max(max_volume, tick.volume)
